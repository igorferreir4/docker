...
func (c *Config) PrintConfig() {
	...
	if c.Mode == "host" {
		...
		log.Println("  Porta: Label Traefik validada contra porta exposta no host")
		log.Println("  Label port: Incluída nas tags")
	} else {
		log.Println("  IP: Container (dinâmico)")
		log.Println("  Porta: Label Traefik validada contra porta interna do container")
		log.Println("  Label port: Incluída nas tags")
	}
...
func (s *ServiceExtractor) HasInternalPort(containerInfo *client.ContainerInspectResult, labelPort int) bool {
	if containerInfo.Container.NetworkSettings == nil {
		return false
	}
	...
	if _, exists := containerInfo.Container.NetworkSettings.Ports[portKey]; exists {
		return true
	}
	...
	_, exists := containerInfo.Container.Config.ExposedPorts[portKey]
	return exists
}

func (s *ServiceExtractor) HasHostPort(containerInfo *client.ContainerInspectResult, labelPort int) bool {
	if containerInfo.Container.NetworkSettings == nil {
		return false
	}

	for _, bindings := range containerInfo.Container.NetworkSettings.Ports {
		for _, binding := range bindings {
			hostPort, err := strconv.Atoi(binding.HostPort)
			if err == nil && hostPort == labelPort {
				return true
			}
		}
	}
...
func traefikObjectName(key, prefix string) (string, bool) {
...
func routerTargetsService(labels map[string]string, routerName, serviceName string) bool {
	if routerName == serviceName {
		return true
	}

	routerServiceKey := fmt.Sprintf("traefik.http.routers.%s.service", routerName)
	return labels[routerServiceKey] == serviceName
}
...
func (s *ServiceExtractor) BuildServiceTags(labels map[string]string, serviceName string) []string {
	...
	for key, value := range labels {
		...
		if traefikServiceName, ok := traefikObjectName(key, "traefik.http.services."); ok {
			if traefikServiceName == serviceName {
				tags = append(tags, fmt.Sprintf("%s=%s", key, value))
			}
			continue
		}

		if routerName, ok := traefikObjectName(key, "traefik.http.routers."); ok {
			if routerTargetsService(labels, routerName, serviceName) {
				tags = append(tags, fmt.Sprintf("%s=%s", key, value))
			}
			continue
		}

		// Labels globais, middlewares e outras configuracoes Traefik do container.
		tags = append(tags, fmt.Sprintf("%s=%s", key, value))
	}
...
func (s *ServiceExtractor) ExtractServicesFromContainer(containerInfo *client.ContainerInspectResult) []ServiceInfo {
	...
	for serviceName, labelPort := range services {
		...
		if s.config.Mode == "host" {
			...
			if !s.HasHostPort(containerInfo, labelPort) {
				log.Printf("%s/%s: Porta da label Traefik %d não está exposta no host",
			...
			port = labelPort
			if !s.HasInternalPort(containerInfo, labelPort) {
				log.Printf("%s/%s: Porta da label Traefik %d não está exposta internamente no container",
					containerInfo.Container.Name, serviceName, labelPort)
				continue
			}
		}
