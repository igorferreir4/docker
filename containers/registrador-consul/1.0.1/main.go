...
func (s *ServiceExtractor) shouldRegisterLabelAsTag(key string) bool {
	return s.config.Mode == "container" || !strings.HasSuffix(key, ".loadbalancer.server.port")
}
...
func (s *ServiceExtractor) BuildServiceTags(labels map[string]string, serviceName string) []string {
	...
	for key, value := range labels {
		...
		if traefikServiceName, ok := traefikObjectName(key, "traefik.http.services."); ok {
			if traefikServiceName == serviceName && s.shouldRegisterLabelAsTag(key) {
